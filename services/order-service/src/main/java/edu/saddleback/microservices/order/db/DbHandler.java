...
public class DbHandler {
    ...
    public JsonObject addOrder(String parsableJson){
        ...
        try{
            ...
            ResultSet rs = connection.prepareStatement("SELECT*FROM users WHERE id = " + param.get("customer_id")).executeQuery();
            if (rs.getString("name") == null)
                throw new Exception();

            ResultSet rs2 = connection.prepareStatement("SELECT*FROM products WHERE id = " + param.get("product_id")).executeQuery();

            connection.prepareStatement("INSERT INTO orders (status, customer_id, cart, coin, address, price)"
                    + "VALUES (PENDING," + param.get("customer_id") + ",NULL, BITCOIN???,peepoo@gmail.com,100)").execute();


            ResultSet rs3 = connection.prepareStatement("SELECT * FROM orders WHERE customer_id = " + param.get("customer_id)")).executeQuery();
